...
contract UniswapV3Staker is IUniswapV3Staker, Multicall {
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        ...
        uint256 size;
        address rewardTokenAddress = address(key.rewardToken);
        assembly {
            size := extcodesize(rewardTokenAddress)
        }
        require(size > 0, "UniswapV3Staker::createIncentive: reward token must be a contract");
        ...
        uint256 rewardBalance = IERC20Minimal(address(key.rewardToken)).balanceOf(address(this));
        require(rewardBalance >= reward, "UniswapV3Staker::createIncentive: reward token balance is insufficient");

        emit IncentiveCreated(
            key.rewardToken, key.pool, key.startTime, key.endTime, key.vestingPeriod, key.refundee, reward
        );
    }
    ...
    function onERC721Received(address, address from, uint256 tokenId, bytes calldata data)
        ...
        returns (bytes4)
    {
        require(msg.sender == address(nonfungiblePositionManager), "UniswapV3Staker::onERC721Received: not a univ3 nft");
